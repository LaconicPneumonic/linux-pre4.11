...
type Dialer struct {
	...
	// Mangler allows the underlying socket to be modified prior
	// to connection establishment.
	//
	// Note that the mangler must be an unnamed type since the
	// gccgo compiler may not be aware of this type. As such, code that
	// uses reflect to assign this field can only assign with an
	// unnamed type.
	Mangler func(fd int) error
}

type socketMangler func(fd int) error
...
func (d *Dialer) DialContext(ctx context.Context, network, address string) (Conn, error) {
	...
	if err := checkDial(network, address); err != nil {
		return nil, err
	}
	...
	if d.Mangler != nil {
		ctx = context.WithValue(ctx, &manglerKey, socketMangler(d.Mangler))
	}
